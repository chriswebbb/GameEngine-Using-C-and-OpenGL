...
gameEngineState engineState = { WINDOWHEIGHT, 
                                WINDOWWIDTH, 
                                0, 
                                0.0f};//engineState = {.toggleWireMesh = 1};
cameraState camera = {  -90.0f,
                         0.0f, 
...
int main(void)
{
    ...
    const GLint mvp_location_three = glGetUniformLocation(program_two, "MVP");        //Returns the location of a uniform variable
    const GLint vpos_location_three = glGetAttribLocation(program_two, "vPos");
    const GLint vcol_location_three = glGetAttribLocation(program_two, "vCol");
    const GLint u_time_location_three = glGetUniformLocation(program_two, "u_time");

    //1. Generate and bind vertex buffer object and copy the vertex data into the buffer
    GLuint vertex_buffer_three;
    glGenBuffers(1, &vertex_buffer_three);                                            
    glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer_three);
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), axis, GL_DYNAMIC_DRAW);
    //2. Generate and bind vertex array
    GLuint vertex_array_three;                                                    
    glGenVertexArrays(1, &vertex_array_three);                                    
    glBindVertexArray(vertex_array_three);                                        
    glEnableVertexAttribArray(vpos_location_three);
    glVertexAttribPointer(vpos_location_three, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*) offsetof(Vertex, pos));
    glEnableVertexAttribArray(vcol_location_three);
    glVertexAttribPointer(vcol_location_three, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*) offsetof(Vertex, col));
    //3. Generate and bind the element buffer object and then populate the buffer
    unsigned int EBO_three;
    glGenBuffers(1, &EBO_three);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO_three);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(axisIndices), axisIndices, GL_DYNAMIC_DRAW);
    ...
    {   
        ...
        glUseProgram(program_two);
        glUniformMatrix4fv(mvp_location_three, 1, GL_FALSE, (const GLfloat*) &mvp);
        glUniform1f(u_time_location_three, u_time);
        glBindVertexArray(vertex_array_three);
